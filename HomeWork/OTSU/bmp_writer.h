...
void save_bmp_test() {
    // Example: Create a 100x100 grayscale image with a gradient pattern
    int width = 100;
    int height = 100;
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            // Create a diagonal gradient from 0 (black) to 255 (white)
            pixels[y * width + x] = (x + y) * 255 / (width + height);
        }
    }
    
    // Save the image as a BMP file
    save_bmp("gradient.bmp", pixels, width, height);
    
    // Clean up
    free(pixels);
    printf("BMP file 'gradient.bmp' created successfully.\n");
}